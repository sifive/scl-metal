/**
 * @file test_hca_sha_384.c
 * @brief test suite for hca_sha.c on sha 384 algorithm
 * @note These tests use HCA (Hardware Cryptographic Accelerator)
 *
 * @copyright Copyright (c) 2020 SiFive, Inc
 * @copyright SPDX-License-Identifier: MIT
 *
 */

#include "unity.h"
#include "unity_fixture.h"

#include <string.h>

#include <backend/api/hash/sha/sha.h>
#include <backend/api/scl_backend_api.h>
#include <backend/hardware/scl_hca.h>

#include  <metal/machine/platform.h>

#if METAL_SIFIVE_HCA_VERSION >= HCA_VERSION(0, 5, 0)

static const metal_scl_t scl = {.hca_base = METAL_SIFIVE_HCA_0_BASE_ADDRESS,
                                .hash_func = {
                                    .sha_init = hca_sha_init,
                                    .sha_core = hca_sha_core,
                                    .sha_finish = hca_sha_finish,
                                }};

TEST_GROUP(hca_sha_384);

TEST_SETUP(hca_sha_384) {}

TEST_TEAR_DOWN(hca_sha_384) {}

TEST(hca_sha_384, msg_abc_all_aligned)
{
    int32_t result = 0;
    sha_ctx_t ctx;

    static const uint8_t message[] __attribute__((aligned(8))) = {
        0x61,
        0x62,
        0x63,
    };

    uint8_t digest[SHA384_BYTE_HASHSIZE] __attribute__((aligned(8)));
    size_t digest_len = sizeof(digest);

    static const uint8_t expected_digest[SHA384_BYTE_HASHSIZE] = {
        0xCB, 0x00, 0x75, 0x3F, 0x45, 0xA3, 0x5E, 0x8B, 0xB5, 0xA0, 0x3D, 0x69,
        0x9A, 0xC6, 0x50, 0x07, 0x27, 0x2C, 0x32, 0xAB, 0x0E, 0xDE, 0xD1, 0x63,
        0x1A, 0x8B, 0x60, 0x5A, 0x43, 0xFF, 0x5B, 0xED, 0x80, 0x86, 0x07, 0x2B,
        0xA1, 0xE7, 0xCC, 0x23, 0x58, 0xBA, 0xEC, 0xA1, 0x34, 0xC8, 0x25, 0xA7};

    result = hca_sha_init(&scl, &ctx, SCL_HASH_SHA384, SCL_BIG_ENDIAN_MODE);
    TEST_ASSERT_TRUE(0 == result);

    result = hca_sha_core(&scl, &ctx, message, sizeof(message));
    TEST_ASSERT_TRUE(0 == result);

    result = hca_sha_finish(&scl, &ctx, digest, &digest_len);
    TEST_ASSERT_TRUE(0 == result);
    TEST_ASSERT_TRUE(SHA384_BYTE_HASHSIZE == digest_len);
    TEST_ASSERT_TRUE(0 ==
                     memcmp(expected_digest, digest, sizeof(expected_digest)));
}

TEST(hca_sha_384, msg_2_blocks_all_aligned)
{
    int32_t result = 0;
    sha_ctx_t ctx;

    static const uint8_t message[] __attribute__((aligned(8))) =
        "abcdefghbcdefghicdefghijdefghijkefghijklfghijklmghijklmnhijklmnoijklmn"
        "opjklmnopqklmnopqrlmnopqrsmnopqrstnopqrstu";

    uint8_t digest[SHA384_BYTE_HASHSIZE] __attribute__((aligned(8)));
    size_t digest_len = sizeof(digest);

    static const uint8_t expected_digest[SHA384_BYTE_HASHSIZE] = {
        0x09, 0x33, 0x0C, 0x33, 0xF7, 0x11, 0x47, 0xE8, 0x3D, 0x19, 0x2F, 0xC7,
        0x82, 0xCD, 0x1B, 0x47, 0x53, 0x11, 0x1B, 0x17, 0x3B, 0x3B, 0x05, 0xD2,
        0x2F, 0xA0, 0x80, 0x86, 0xE3, 0xB0, 0xF7, 0x12, 0xFC, 0xC7, 0xC7, 0x1A,
        0x55, 0x7E, 0x2D, 0xB9, 0x66, 0xC3, 0xE9, 0xFA, 0x91, 0x74, 0x60, 0x39};

    result = hca_sha_init(&scl, &ctx, SCL_HASH_SHA384, SCL_BIG_ENDIAN_MODE);
    TEST_ASSERT_TRUE(0 == result);

    result = hca_sha_core(&scl, &ctx, message, sizeof(message) - 1);
    TEST_ASSERT_TRUE(0 == result);

    result = hca_sha_finish(&scl, &ctx, digest, &digest_len);
    TEST_ASSERT_TRUE(0 == result);
    TEST_ASSERT_TRUE(SHA384_BYTE_HASHSIZE == digest_len);
    TEST_ASSERT_TRUE(0 ==
                     memcmp(expected_digest, digest, sizeof(expected_digest)));
}

TEST(hca_sha_384, msg_abc_msg_not_aligned)
{
    int32_t result = 0;
    sha_ctx_t ctx;

    static const uint8_t message[] __attribute__((aligned(8))) = {
        0x00,
        0x61,
        0x62,
        0x63,
    };

    uint8_t digest[SHA384_BYTE_HASHSIZE] __attribute__((aligned(8)));
    size_t digest_len = sizeof(digest);

    static const uint8_t expected_digest[SHA384_BYTE_HASHSIZE] = {
        0xCB, 0x00, 0x75, 0x3F, 0x45, 0xA3, 0x5E, 0x8B, 0xB5, 0xA0, 0x3D, 0x69,
        0x9A, 0xC6, 0x50, 0x07, 0x27, 0x2C, 0x32, 0xAB, 0x0E, 0xDE, 0xD1, 0x63,
        0x1A, 0x8B, 0x60, 0x5A, 0x43, 0xFF, 0x5B, 0xED, 0x80, 0x86, 0x07, 0x2B,
        0xA1, 0xE7, 0xCC, 0x23, 0x58, 0xBA, 0xEC, 0xA1, 0x34, 0xC8, 0x25, 0xA7};

    result = hca_sha_init(&scl, &ctx, SCL_HASH_SHA384, SCL_BIG_ENDIAN_MODE);
    TEST_ASSERT_TRUE(0 == result);

    result = hca_sha_core(&scl, &ctx, &message[1], sizeof(message) - 1);
    TEST_ASSERT_TRUE(0 == result);

    result = hca_sha_finish(&scl, &ctx, digest, &digest_len);
    TEST_ASSERT_TRUE(0 == result);
    TEST_ASSERT_TRUE(SHA384_BYTE_HASHSIZE == digest_len);
    TEST_ASSERT_TRUE(0 ==
                     memcmp(expected_digest, digest, sizeof(expected_digest)));
}

TEST(hca_sha_384, msg_2_blocks_msg_not_aligned)
{
    int32_t result = 0;
    sha_ctx_t ctx;

    static const uint8_t message[] __attribute__((aligned(8))) =
        "aabcdefghbcdefghicdefghijdefghijkefghijklfghijklmghijklmnhijklmnoijklm"
        "nopjklmnopqklmnopqrlmnopqrsmnopqrstnopqrstu";

    uint8_t digest[SHA384_BYTE_HASHSIZE] __attribute__((aligned(8)));
    size_t digest_len = sizeof(digest);

    static const uint8_t expected_digest[SHA384_BYTE_HASHSIZE] = {
        0x09, 0x33, 0x0C, 0x33, 0xF7, 0x11, 0x47, 0xE8, 0x3D, 0x19, 0x2F, 0xC7,
        0x82, 0xCD, 0x1B, 0x47, 0x53, 0x11, 0x1B, 0x17, 0x3B, 0x3B, 0x05, 0xD2,
        0x2F, 0xA0, 0x80, 0x86, 0xE3, 0xB0, 0xF7, 0x12, 0xFC, 0xC7, 0xC7, 0x1A,
        0x55, 0x7E, 0x2D, 0xB9, 0x66, 0xC3, 0xE9, 0xFA, 0x91, 0x74, 0x60, 0x39};

    result = hca_sha_init(&scl, &ctx, SCL_HASH_SHA384, SCL_BIG_ENDIAN_MODE);
    TEST_ASSERT_TRUE(0 == result);

    result = hca_sha_core(&scl, &ctx, &message[1], sizeof(message) - 2);
    TEST_ASSERT_TRUE(0 == result);

    result = hca_sha_finish(&scl, &ctx, digest, &digest_len);
    TEST_ASSERT_TRUE(0 == result);
    TEST_ASSERT_TRUE(SHA384_BYTE_HASHSIZE == digest_len);
    TEST_ASSERT_TRUE(0 ==
                     memcmp(expected_digest, digest, sizeof(expected_digest)));
}

TEST(hca_sha_384, msg_abc_digest_not_aligned)
{
    int32_t result = 0;
    sha_ctx_t ctx;

    static const uint8_t message[] __attribute__((aligned(8))) = {
        0x61,
        0x62,
        0x63,
    };

    uint8_t digest[SHA384_BYTE_HASHSIZE + 1] __attribute__((aligned(8)));
    size_t digest_len = sizeof(digest) - 1;

    static const uint8_t expected_digest[SHA384_BYTE_HASHSIZE] = {
        0xCB, 0x00, 0x75, 0x3F, 0x45, 0xA3, 0x5E, 0x8B, 0xB5, 0xA0, 0x3D, 0x69,
        0x9A, 0xC6, 0x50, 0x07, 0x27, 0x2C, 0x32, 0xAB, 0x0E, 0xDE, 0xD1, 0x63,
        0x1A, 0x8B, 0x60, 0x5A, 0x43, 0xFF, 0x5B, 0xED, 0x80, 0x86, 0x07, 0x2B,
        0xA1, 0xE7, 0xCC, 0x23, 0x58, 0xBA, 0xEC, 0xA1, 0x34, 0xC8, 0x25, 0xA7};

    result = hca_sha_init(&scl, &ctx, SCL_HASH_SHA384, SCL_BIG_ENDIAN_MODE);
    TEST_ASSERT_TRUE(0 == result);

    result = hca_sha_core(&scl, &ctx, message, sizeof(message));
    TEST_ASSERT_TRUE(0 == result);

    result = hca_sha_finish(&scl, &ctx, &digest[1], &digest_len);
    TEST_ASSERT_TRUE(0 == result);
    TEST_ASSERT_TRUE(SHA384_BYTE_HASHSIZE == digest_len);
    TEST_ASSERT_TRUE(
        0 == memcmp(expected_digest, &digest[1], sizeof(expected_digest)));
}

TEST(hca_sha_384, msg_2_blocks_digest_not_aligned)
{
    int32_t result = 0;
    sha_ctx_t ctx;

    static const uint8_t message[] __attribute__((aligned(8))) =
        "abcdefghbcdefghicdefghijdefghijkefghijklfghijklmghijklmnhijklmnoijklmn"
        "opjklmnopqklmnopqrlmnopqrsmnopqrstnopqrstu";

    uint8_t digest[SHA384_BYTE_HASHSIZE + 1] __attribute__((aligned(8)));
    size_t digest_len = sizeof(digest) - 1;

    static const uint8_t expected_digest[SHA384_BYTE_HASHSIZE] = {
        0x09, 0x33, 0x0C, 0x33, 0xF7, 0x11, 0x47, 0xE8, 0x3D, 0x19, 0x2F, 0xC7,
        0x82, 0xCD, 0x1B, 0x47, 0x53, 0x11, 0x1B, 0x17, 0x3B, 0x3B, 0x05, 0xD2,
        0x2F, 0xA0, 0x80, 0x86, 0xE3, 0xB0, 0xF7, 0x12, 0xFC, 0xC7, 0xC7, 0x1A,
        0x55, 0x7E, 0x2D, 0xB9, 0x66, 0xC3, 0xE9, 0xFA, 0x91, 0x74, 0x60, 0x39};

    result = hca_sha_init(&scl, &ctx, SCL_HASH_SHA384, SCL_BIG_ENDIAN_MODE);
    TEST_ASSERT_TRUE(0 == result);

    result = hca_sha_core(&scl, &ctx, message, sizeof(message) - 1);
    TEST_ASSERT_TRUE(0 == result);

    result = hca_sha_finish(&scl, &ctx, &digest[1], &digest_len);
    TEST_ASSERT_TRUE(0 == result);
    TEST_ASSERT_TRUE(SHA384_BYTE_HASHSIZE == digest_len);
    TEST_ASSERT_TRUE(
        0 == memcmp(expected_digest, &digest[1], sizeof(expected_digest)));
}

TEST(hca_sha_384, msg_1024_bytes_aligned)
{
    int32_t result = 0;
    sha_ctx_t ctx;

    static const uint8_t message[] __attribute__((aligned(8))) = {
        };

    uint8_t digest[SHA384_BYTE_HASHSIZE] __attribute__((aligned(8)));
    size_t digest_len = sizeof(digest);

    static const uint8_t expected_digest[SHA384_BYTE_HASHSIZE] = {
        0x3B, 0xF4, 0xC0, 0xCB, 0xD5, 0x5E, 0x0B, 0xBE, 0x21, 0x4A, 0x42, 0xFE,
        0x96, 0x85, 0x8E, 0xCB, 0x67, 0xFE, 0x91, 0x2D, 0x74, 0xFB, 0xCF, 0x8F,
        0xB6, 0xD0, 0xAF, 0x06, 0x25, 0x34, 0x0C, 0x17, 0xB1, 0x2F, 0x1F, 0xFF,
        0x99, 0xC2, 0x69, 0x1D, 0x31, 0xCE, 0x13, 0x00, 0x1C, 0x90, 0x8C, 0x11};

    result = hca_sha_init(&scl, &ctx, SCL_HASH_SHA384, SCL_BIG_ENDIAN_MODE);
    TEST_ASSERT_TRUE(0 == result);

    result = hca_sha_core(&scl, &ctx, message, sizeof(message));
    TEST_ASSERT_TRUE(0 == result);

    result = hca_sha_finish(&scl, &ctx, digest, &digest_len);
    TEST_ASSERT_TRUE(0 == result);
    TEST_ASSERT_TRUE(SHA384_BYTE_HASHSIZE == digest_len);
    TEST_ASSERT_TRUE(0 ==
                     memcmp(expected_digest, digest, sizeof(expected_digest)));
}

TEST(hca_sha_384, msg_1024_bytes_not_aligned)
{
    int32_t result = 0;
    sha_ctx_t ctx;

    static const uint8_t message[] __attribute__((aligned(8))) = {
        };

    uint8_t digest[SHA384_BYTE_HASHSIZE] __attribute__((aligned(8)));
    size_t digest_len = sizeof(digest);

    static const uint8_t expected_digest[SHA384_BYTE_HASHSIZE] = {
        0x3B, 0xF4, 0xC0, 0xCB, 0xD5, 0x5E, 0x0B, 0xBE, 0x21, 0x4A, 0x42, 0xFE,
        0x96, 0x85, 0x8E, 0xCB, 0x67, 0xFE, 0x91, 0x2D, 0x74, 0xFB, 0xCF, 0x8F,
        0xB6, 0xD0, 0xAF, 0x06, 0x25, 0x34, 0x0C, 0x17, 0xB1, 0x2F, 0x1F, 0xFF,
        0x99, 0xC2, 0x69, 0x1D, 0x31, 0xCE, 0x13, 0x00, 0x1C, 0x90, 0x8C, 0x11};

    result = hca_sha_init(&scl, &ctx, SCL_HASH_SHA384, SCL_BIG_ENDIAN_MODE);
    TEST_ASSERT_TRUE(0 == result);

    result = hca_sha_core(&scl, &ctx, &message[1], sizeof(message) - 1);
    TEST_ASSERT_TRUE(0 == result);

    result = hca_sha_finish(&scl, &ctx, digest, &digest_len);
    TEST_ASSERT_TRUE(0 == result);
    TEST_ASSERT_TRUE(SHA384_BYTE_HASHSIZE == digest_len);
    TEST_ASSERT_TRUE(0 ==
                     memcmp(expected_digest, digest, sizeof(expected_digest)));
}

TEST(hca_sha_384, msg_192B_digest_aligned)
{
    int32_t result = 0;
    sha_ctx_t sha_ctx;

    static const uint8_t message[] __attribute__((aligned(8))) = {
        };

    uint8_t digest[SHA384_BYTE_HASHSIZE] __attribute__((aligned(8)));
    size_t digest_len = sizeof(digest);

    static const uint8_t expected_digest[SHA384_BYTE_HASHSIZE] = {
        0x1C, 0xF9, 0x97, 0x8D, 0xDC, 0x30, 0xB7, 0x3E, 0x4A, 0x2B, 0x26, 0x9C,
        0x5C, 0xC0, 0xBC, 0x77, 0x6E, 0xF6, 0xF3, 0xAF, 0x94, 0x60, 0xFB, 0x54,
        0xE3, 0x11, 0x24, 0x34, 0xEB, 0x92, 0x97, 0xA9, 0xEC, 0x6D, 0xFC, 0x80,
        0xCC, 0xE1, 0xE9, 0x12, 0xEA, 0x60, 0xBB, 0x3B, 0x60, 0xE5, 0x03, 0xB2};

    result = hca_sha_init(&scl, &sha_ctx, SCL_HASH_SHA384, SCL_BIG_ENDIAN_MODE);
    TEST_ASSERT_TRUE(0 == result);

    /* here uncomplete block */
    result =
        hca_sha_core(&scl, &sha_ctx, message, SHA512_BYTE_BLOCKSIZE / 2 - 1);

    TEST_ASSERT_TRUE(0 == result);

    result =
        hca_sha_core(&scl, &sha_ctx, &message[SHA512_BYTE_BLOCKSIZE / 2 - 1],
                     sizeof(message) - (SHA512_BYTE_BLOCKSIZE / 2 - 1));

    TEST_ASSERT_TRUE(0 == result);

    result = hca_sha_finish(&scl, &sha_ctx, digest, &digest_len);

    TEST_ASSERT_TRUE(0 == result);
    TEST_ASSERT_TRUE(SHA384_BYTE_HASHSIZE == digest_len);
    TEST_ASSERT_TRUE(0 ==
                     memcmp(expected_digest, digest, sizeof(expected_digest)));
}

TEST(hca_sha_384, msg_2_block_and_half_digest_aligned)
{
    int32_t result = 0;
    sha_ctx_t sha_ctx;

    static const uint8_t message[] __attribute__((aligned(8))) = {
        };

    uint8_t digest[SHA384_BYTE_HASHSIZE] __attribute__((aligned(8)));
    size_t digest_len = sizeof(digest);

    static const uint8_t expected_digest[SHA384_BYTE_HASHSIZE] = {
        0xD9, 0xC7, 0x27, 0xC9, 0x85, 0x27, 0xAF, 0x44, 0x83, 0x67, 0xB1, 0x02,
        0x8C, 0xB7, 0x7F, 0x41, 0x66, 0x46, 0x2C, 0xD3, 0x2F, 0x27, 0xEA, 0xAD,
        0x57, 0xE8, 0xCC, 0xA2, 0x4E, 0x38, 0x8B, 0xAB, 0xFC, 0x8A, 0x7C, 0xE0,
        0x73, 0x79, 0x0D, 0x52, 0x38, 0x07, 0x87, 0x60, 0x09, 0xA9, 0xA8, 0x71};

    result = hca_sha_init(&scl, &sha_ctx, SCL_HASH_SHA384, SCL_BIG_ENDIAN_MODE);
    TEST_ASSERT_TRUE(0 == result);

    /* here uncomplete block */
    result =
        hca_sha_core(&scl, &sha_ctx, message, SHA512_BYTE_BLOCKSIZE / 2 - 1);

    TEST_ASSERT_TRUE(0 == result);

    result =
        hca_sha_core(&scl, &sha_ctx, &message[SHA512_BYTE_BLOCKSIZE / 2 - 1],
                     sizeof(message) - (SHA512_BYTE_BLOCKSIZE / 2 - 1));

    TEST_ASSERT_TRUE(0 == result);

    result = hca_sha_finish(&scl, &sha_ctx, digest, &digest_len);

    TEST_ASSERT_TRUE(0 == result);
    TEST_ASSERT_TRUE(SHA384_BYTE_HASHSIZE == digest_len);
    TEST_ASSERT_TRUE(0 ==
                     memcmp(expected_digest, digest, sizeof(expected_digest)));
}

TEST(hca_sha_384, msg_and_hash_twice)
{
    int32_t result = 0;
    sha_ctx_t sha_ctx;

    static const uint8_t message[] = {
        };

    static const uint8_t message_2[] = {
        };

    uint8_t digest[SHA384_BYTE_HASHSIZE] __attribute__((aligned(8)));
    size_t digest_len = sizeof(digest);

    static const uint8_t expected_digest[SHA384_BYTE_HASHSIZE] = {
        0xD9, 0xC7, 0x27, 0xC9, 0x85, 0x27, 0xAF, 0x44, 0x83, 0x67, 0xB1, 0x02,
        0x8C, 0xB7, 0x7F, 0x41, 0x66, 0x46, 0x2C, 0xD3, 0x2F, 0x27, 0xEA, 0xAD,
        0x57, 0xE8, 0xCC, 0xA2, 0x4E, 0x38, 0x8B, 0xAB, 0xFC, 0x8A, 0x7C, 0xE0,
        0x73, 0x79, 0x0D, 0x52, 0x38, 0x07, 0x87, 0x60, 0x09, 0xA9, 0xA8, 0x71};

    static const uint8_t expected_digest_2[SHA384_BYTE_HASHSIZE] = {
        0x69, 0xC6, 0x0D, 0x51, 0xB8, 0xF6, 0xEB, 0xF2, 0x93, 0xCD, 0xED, 0x81,
        0xEA, 0x05, 0x67, 0x07, 0xF5, 0x8A, 0x21, 0xF6, 0x89, 0xED, 0x3A, 0xF2,
        0xE3, 0xFA, 0xF9, 0x5E, 0xDF, 0x89, 0x24, 0x2A, 0x64, 0x94, 0x77, 0x42,
        0x7C, 0x9F, 0x05, 0xDB, 0x50, 0x7B, 0xBD, 0x6E, 0x96, 0x98, 0x6B, 0x77};

    result = hca_sha_init(&scl, &sha_ctx, SCL_HASH_SHA384, SCL_BIG_ENDIAN_MODE);
    TEST_ASSERT_TRUE(0 == result);

    /* here uncomplete block */
    result =
        hca_sha_core(&scl, &sha_ctx, message, SHA512_BYTE_BLOCKSIZE / 2 - 1);

    TEST_ASSERT_TRUE(0 == result);

    result =
        hca_sha_core(&scl, &sha_ctx, &message[SHA512_BYTE_BLOCKSIZE / 2 - 1],
                     sizeof(message) - (SHA512_BYTE_BLOCKSIZE / 2 - 1));

    TEST_ASSERT_TRUE(0 == result);

    result = hca_sha_finish(&scl, &sha_ctx, digest, &digest_len);

    TEST_ASSERT_TRUE(0 == result);
    TEST_ASSERT_TRUE(SHA384_BYTE_HASHSIZE == digest_len);
    TEST_ASSERT_TRUE(0 ==
                     memcmp(expected_digest, digest, sizeof(expected_digest)));

    result = hca_sha_init(&scl, &sha_ctx, SCL_HASH_SHA384, SCL_BIG_ENDIAN_MODE);
    TEST_ASSERT_TRUE(0 == result);

    /* here uncomplete block */
    result =
        hca_sha_core(&scl, &sha_ctx, message_2, SHA512_BYTE_BLOCKSIZE / 2 - 1);

    TEST_ASSERT_TRUE(0 == result);

    result =
        hca_sha_core(&scl, &sha_ctx, &message_2[SHA512_BYTE_BLOCKSIZE / 2 - 1],
                     sizeof(message_2) - (SHA512_BYTE_BLOCKSIZE / 2 - 1));

    TEST_ASSERT_TRUE(0 == result);

    result = hca_sha_finish(&scl, &sha_ctx, digest, &digest_len);

    TEST_ASSERT_TRUE(0 == result);
    TEST_ASSERT_TRUE(SHA384_BYTE_HASHSIZE == digest_len);
    TEST_ASSERT_TRUE(
        0 == memcmp(expected_digest_2, digest, sizeof(expected_digest_2)));
}

TEST(hca_sha_384, msg_1_block_in_3_pieces_digest_aligned)
{
    int32_t result = 0;
    sha_ctx_t sha_ctx;

    static const uint8_t message[] __attribute__((aligned(8))) = {
        };

    uint8_t digest[SHA384_BYTE_HASHSIZE] __attribute__((aligned(8)));
    size_t digest_len = sizeof(digest);

    static const uint8_t expected_digest[SHA384_BYTE_HASHSIZE] = {
        0x74, 0x48, 0x56, 0x52, 0x71, 0xA7, 0xD7, 0x36, 0xC0, 0x61, 0xCF, 0x2F,
        0x36, 0x3E, 0x90, 0xB5, 0x7E, 0x0E, 0x2D, 0xD6, 0x56, 0x3C, 0x2B, 0xCF,
        0xB6, 0xE4, 0xB9, 0xB1, 0xC1, 0x74, 0xBB, 0xE9, 0xFE, 0x59, 0xBF, 0xA4,
        0x1D, 0x53, 0x1B, 0x8E, 0x95, 0x54, 0xBF, 0xFC, 0xFF, 0xA2, 0xA0, 0x9E};

    result = hca_sha_init(&scl, &sha_ctx, SCL_HASH_SHA384, SCL_BIG_ENDIAN_MODE);
    TEST_ASSERT_TRUE(0 == result);

    /* here uncomplete block */
    result =
        hca_sha_core(&scl, &sha_ctx, message, SHA512_BYTE_BLOCKSIZE / 3 - 1);

    TEST_ASSERT_TRUE(0 == result);

    result =
        hca_sha_core(&scl, &sha_ctx, &message[SHA512_BYTE_BLOCKSIZE / 3 - 1],
                     SHA512_BYTE_BLOCKSIZE / 3 - 1);

    TEST_ASSERT_TRUE(0 == result);

    result = hca_sha_core(
        &scl, &sha_ctx, &message[2 * (SHA512_BYTE_BLOCKSIZE / 3 - 1)],
        sizeof(message) - 2 * (SHA512_BYTE_BLOCKSIZE / 3 - 1));

    TEST_ASSERT_TRUE(0 == result);

    result = hca_sha_finish(&scl, &sha_ctx, digest, &digest_len);

    TEST_ASSERT_TRUE(0 == result);
    TEST_ASSERT_TRUE(SHA384_BYTE_HASHSIZE == digest_len);
    TEST_ASSERT_TRUE(0 ==
                     memcmp(expected_digest, digest, sizeof(expected_digest)));
}

#endif
